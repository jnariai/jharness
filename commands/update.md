...
| Signal | Command |
|---|---|
| installed version | `version` in `${CLAUDE_PLUGIN_ROOT}/.claude-plugin/plugin.json` |
| upstream version + commit | `version` in `$SRC/jharness/.claude-plugin/plugin.json`; `git -C "$SRC/jharness" rev-parse --short=12 HEAD` |
| content drift | `diff -rq --exclude=.git "${CLAUDE_PLUGIN_ROOT}" "$SRC/jharness"` — empty = identical |
...
`${CLAUDE_PLUGIN_ROOT}`. Continue with step 4.
...
Never edit, pull, or copy files inside `${CLAUDE_PLUGIN_ROOT}` or
...
```bash
"${CLAUDE_PLUGIN_ROOT}/scripts/sync-guidelines.sh" --source "$SRC/jharness" --check <target>
```
...
diff -u <target>/docs/agents/coding_guidelines.md \
  <("${CLAUDE_PLUGIN_ROOT}/scripts/sync-guidelines.sh" --source "$SRC/jharness" --print-upstream <target>)
```
